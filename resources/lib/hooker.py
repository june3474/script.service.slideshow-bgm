...
class Hooker():
    ...
    def __init__(self):
        self.target = self.find_SlideShow_xml()
    ...
    def check_hooked(self):
        """Check current skin's SlideShow.xml is hooked up to ``slideshow-BGM`` of the current
        skin.

        Technically, check ``<onload *with ``self.element.attrib``*>RunAddon(script.slideshow-BGM)</onload>``
    ...
    def insert_tag(self):
        """Insert tag ``<onload *with ``self.element.attrib``*>RunAddon(script.slideshow-BGM)</onload>``
        into `SlideShow.xml` file of the current skin.
    ...
    def remove_tag(self):
        """Remove tag ``<onload> RunAddon(script.slideshow-BGM) </onload>``
        from `SlideShow.xml` file of the current skin.
